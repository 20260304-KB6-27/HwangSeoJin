<template>
  <div id="app" class="container">
    <h1 id="title">📌 게시판 목록 v4</h1>
    <InputPost @add="addHandler" />
    <ul>
      <PostItem
        @edit="editHandler"
...
const posts = computed(() => {
  return rawPosts.value.sort((a, b) => b.no - a.no);
});

const editHandler = (editContent) => {
  let { title, content, no } = editContent;
  let index = posts.value.findIndex((item) => item.no === no);
  posts.value.splice(index, 1, editContent);
};
